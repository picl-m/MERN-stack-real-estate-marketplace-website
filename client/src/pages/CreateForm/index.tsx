...
export default function CreateForm(props: CreateFormProps) {
  ...
  const submitForm = () => {
    ...
    if (formData.phone) formattedData.phone = formData.phone.replace(/\s/g, "");

    const [valid, error] = validateEstate(formattedData, props.estateType);

    if (valid) {
      createEstate(formattedData as Estate, props.estateType).then(
        (success) => {
          if (success) {
            navigate("/");
          }
      ...
      {loading === false ? (
        <Container maxWidth="md" sx={{ py: 4 }}>
          <Typography variant="h3" paragraph>
            {"Create new " + props.estateType + " listing"}
          </Typography>
          <Paper
            sx={{
              minHeight: "500px",
