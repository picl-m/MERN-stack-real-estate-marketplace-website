...
export default function Search(props: SearchProps) {
  const [estateTypeAnchor, setEstateTypeAnchor] = useState<null | HTMLElement>(
    null,
  );
  ...
  const displayEstateType =
    props.estateType.charAt(0).toUpperCase() + props.estateType.slice(1);
  ...
  const goToResults = () => {
    let resultsSearchParams = createSearchParams(
    ...
    <Layout>
      <Container sx={{ py: 4 }} maxWidth="md">
        <Stack direction="row" alignItems="center" paddingBottom={1}>
          <Typography variant="h4">{displayEstateType}</Typography>
          <IconButton
            aria-label="more"
          ...
          <Menu
            id="estate-type-menu"
            MenuListProps={{
              "aria-labelledby": "estate-type-button",
            }}
